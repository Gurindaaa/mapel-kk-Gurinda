  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header bg-warning">
        <h5 class="modal-title">Edit Informasi Foto</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
      </div>
...
// Simpan perubahan Edit (hanya di tampilan)
document.getElementById('editForm').addEventListener('submit', function(e) {
  e.preventDefault();
  const index = document.getElementById('editIndex').value;
  const judulBaru = document.getElementById('editJudul').value;
  const deskripsiBaru = document.getElementById('editDeskripsi').value;

  const item = document.querySelector(.gallery-item[data-index="${index}"]);
  item.querySelector('.judul').textContent = judulBaru;
  item.querySelector('.deskripsi').textContent = deskripsiBaru;
